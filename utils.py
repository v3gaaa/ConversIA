...
def vector_qa(direc_marca):
    ...
    qa_marca = RetrievalQA.from_chain_type(llm=ChatOpenAI(openai_api_key= config("OPENAI_API_KEY")), chain_type= "stuff", retriever = vectordb.as_retriever(), chain_type_kwargs=chain_type_kwargs)
    
    return qa_marca

def chat_prompt():
    #Template del prompt que va a ser enviado al modelo de texto para que me regrese el mejor mensaje
    template="""Eres un chatbot especializado en atención al cliente. Tu trabajo es responder al cliente de la manera mas concisa y segura posible. Tu publico esta enfocada hacia la poblacion Mexicana en general, recuerda que muchas veces esta misma tiene diversas formas de preguntar cosas entonces puede que no siempre te den la pregunta de forma clara. Intenta los mas que puedas de manera coherente no responder con respuestas negativas o que no puedes responder algo, sino guialos lo mas que puedas. Intenta personalizar la experiencia lo mas que puedas si el cliente da datos personales como su genero, edad, ocupacion,necesidades,etc. recuerdalo para usarlo. Este es el historial de mensajes:
    {chat_history}
...
def chat(pregunta, memory):
    ...
    chat=ChatOpenAI(temperature=0, model_name="gpt-3.5-turbo",openai_api_key=config("OPENAI_API_KEY"))
